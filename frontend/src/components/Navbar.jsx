...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-900/80 backdrop-blur-xl border-b border-gray-700/50 shadow-lg transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`relative px-4 py-2 rounded-xl font-medium transition-all duration-300 transform hover:scale-105 ${
                  isActive(link.path)
                    ? 'text-blue-400 bg-blue-900/30 shadow-lg shadow-blue-500/10 border border-blue-700'
                    : 'text-gray-300 hover:text-blue-400 hover:bg-gray-800 border border-transparent hover:border-gray-700'
                }`}
              >
          ...
          <div className="md:hidden flex items-center space-x-2">
            <button
              ...
              className="p-2 rounded-xl bg-gray-800 text-gray-300 hover:bg-gray-700 transition-all duration-300"
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden py-4 space-y-2 bg-gray-900/95 backdrop-blur-xl rounded-2xl mt-2 border border-gray-700/50 shadow-xl">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
            ...
            {isLoggedIn && (
              <button
                ...
                className="block w-full text-left px-4 py-3 rounded-xl font-medium text-red-400 hover:bg-gray-800 transition duration-300 mx-2"
              >
                Logout
              </button>
            )}
